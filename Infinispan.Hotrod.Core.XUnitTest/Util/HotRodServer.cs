...
namespace Infinispan.Hotrod.Core.Tests.Util
{
    public class HotRodServer : IDisposable
    {
        ...
        void StartHotrodServerInternal()
        {
            ...
            Console.WriteLine("Command is:" + hrServer.StartInfo.FileName + ": arguments:" + hrServer.StartInfo.Arguments);
            if (PlatformUtils.isUnix())
            {
        ...
        public void ShutDownHotrodServer()
        {
            ...
            if (hrServer != null)
            {
                try
                {
                    ...
                    if (!IsStopped())
                    {
                        throw new Exception("A process is still listening on the ip/port. Kill failed?");
                    }
                    hrServer.Close();
                }
                catch (InvalidOperationException)
                {
                    // shutdown called twice. Ignore it
                }
                started = false;
            }
        }
